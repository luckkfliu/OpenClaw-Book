...
Android 节点应用 $\rightleftarrows$（mDNS/NSD + WebSocket）$\rightleftarrows$ \textbf{Gateway 网关}
...
对于仅 tailnet 设置（推荐用于维也纳 $\rightleftarrows$ 伦敦），将 Gateway 网关绑定到 tailnet IP：
...
\paragraph{通过 unicast DNS-SD 的 Tailnet（维也纳 $\rightleftarrows$ 伦敦）发现}
...
\begin{itemize}
  \item \texttt{exec} → \emoji{💻}
  \item \texttt{read} → \emoji{📄}
  \item \texttt{write} → \emoji{✍}
  \item \texttt{edit} → \emoji{📝}
  \item \texttt{attach} → \emoji{📎}
  \item 默认 → \emoji{🛠}
\end{itemize}
...
\begin{itemize}
  \item 设置 $\triangleright$ 调试 $\triangleright$ "图标覆盖" 选择器：
  \item \texttt{系统（自动）}（默认）
...
\section{远程 OpenClaw（macOS $\rightleftarrows$ 远程主机）}
...
Pi 型号 & 内存 & 是否可用？ & 说明 \\
\hline
\textbf{Pi 5} & 4GB/8GB & \emoji{✅} 最佳 & 最快，推荐 \\
\hline
\textbf{Pi 4} & 4GB & \emoji{✅} 良好 & 大多数用户的最佳选择 \\
\hline
\textbf{Pi 4} & 2GB & \emoji{✅} 可以 & 可用，添加交换空间 \\
\hline
\textbf{Pi 4} & 1GB & \emoji{⚠} 紧张 & 使用交换空间可行，最小配置 \\
\hline
\textbf{Pi 3B+} & 1GB & \emoji{⚠} 慢 & 可用但较慢 \\
\hline
\textbf{Pi Zero 2 W} & 512MB & \emoji{❌} & 不推荐 \\
\hline
\end{tabular}
\end{table}
...
Node.js & \emoji{✅} & 运行良好 \\
\hline
WhatsApp (Baileys) & \emoji{✅} & 纯 JS，无问题 \\
\hline
Telegram & \emoji{✅} & 纯 JS，无问题 \\
\hline
gog (Gmail CLI) & \emoji{⚠} & 检查是否有 ARM 版本 \\
\hline
Chromium (browser) & \emoji{✅} & \texttt{sudo apt install chromium-browser} \\
\hline
\end{tabular}
\end{table}
