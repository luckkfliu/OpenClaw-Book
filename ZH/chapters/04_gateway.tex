...
\begin{itemize}
  \item \texttt{messages.ackReaction} 来自\textbf{活跃智能体}的 \texttt{identity.emoji}（回退到 \emoji{👀}）
...
\texttt{ackReaction} 在支持反应的渠道（Slack/Discord/Telegram/Google Chat）上发送尽力而为的表情反应来确认入站消息。设置时默认为活跃智能体的 \texttt{identity.emoji}，否则为 \texttt{"\emoji{👀}"}。设为 \texttt{""} 禁用。
...
\_下一步：\href{/concepts/agent}{智能体运行时}\_ \emoji{🦞}
...
\begin{itemize}
  ...
  \item 在心跳运行期间，当 \texttt{HEARTBEAT\_OK} 出现在回复的\textbf{开头或结尾}时，OpenClaw 将其视为确认。该标记会被移除，如果剩余内容 \textbf{$\leq$ \texttt{ackMaxChars}}（默认：300），则回复被丢弃。
...
本地运行是可行的，但 OpenClaw 期望大上下文 + 强大的提示注入防御。小显存会截断上下文并泄露安全性。目标要高：\textbf{$\geq$2 台满配 Mac Studio 或同等 GPU 配置（约 \$30k+）}。单张 \textbf{24 GB} GPU 仅适用于较轻的提示，且延迟更高。使用\textbf{你能运行的最大/完整尺寸模型变体}；激进量化或"小型"检查点会增加提示注入风险（参见\href{/gateway/security}{安全}）。
...
详细工具摘要（例如 \texttt{\emoji{🛠} Exec: ...}）可以在进入控制台流之前屏蔽敏感令牌。这\textbf{仅限工具}，不会更改文件日志。
...
\begin{itemize}
  \item \emoji{❌} \texttt{@openclaw} + 图片
  \item \emoji{✅} \texttt{@openclaw check this} + 图片
\end{itemize}
...
\emoji{⚠} 这会丢失所有会话并需要重新配对 WhatsApp。
...
\clearpage
% === 源文件: gateway/configuration-reference.md ===
...
\begin{itemize}
  \item Defaults to active agent's \texttt{identity.emoji}, otherwise \texttt{"\emoji{👀}"}. Set \texttt{""} to disable.
...
\begin{itemize}
  \item \texttt{messages.ackReaction} from \texttt{identity.emoji} (falls back to \emoji{👀})
...
% === 源文件: gateway/secrets-plan-contract.md ===
...
% === 源文件: gateway/trusted-proxy-auth.md ===
\section{Trusted Proxy Auth}


\begin{quote}
\emoji{⚠} \textbf{Security-sensitive feature.} This mode delegates authentication entirely to your reverse proxy. Misconfiguration can expose your Gateway to unauthorized access. Read this page carefully before enabling.
\end{quote}
