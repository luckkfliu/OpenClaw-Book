...
\begin{quote}
\textbf{\emoji{📦} 完整指南：\href{https://github.com/openclaw/openclaw-ansible}{github.com/openclaw/openclaw-ansible}}
...
\begin{itemize}
  \item \emoji{🔒} \textbf{防火墙优先安全}：UFW + Docker 隔离（仅 SSH + Tailscale 可访问）
  \item \emoji{🔐} \textbf{Tailscale VPN}：安全远程访问，无需公开暴露服务
  \item \emoji{🐳} \textbf{Docker}：隔离的沙箱容器，仅绑定 localhost
  \item \emoji{🛡} \textbf{纵深防御}：4 层安全架构
  \item \emoji{🚀} \textbf{一条命令设置}：几分钟内完成部署
  \item \emoji{🔧} \textbf{Systemd 集成}：带加固的开机自启动
\end{itemize}
...
\emoji{⚠} \textbf{不推荐用于 Gateway 网关运行时}（WhatsApp/Telegram 存在 bug）。生产环境请使用 Node。
...
\begin{quote}
\textbf{\emoji{📦} 完整指南：\href{https://github.com/openclaw/nix-openclaw}{github.com/openclaw/nix-openclaw}}
