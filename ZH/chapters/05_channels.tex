...
✅ \textbf{好的做法：} 每个智能体只有一个任务
❌ \textbf{不好的做法：} 一个通用的"dev-helper"智能体
...
\begin{itemize}
  ...
  \item security-scanner："⚠️ 第 12 行存在 SQL 注入漏洞"
...
\begin{itemize}
  \item ✅ \texttt{bot}
  \item ✅ \texttt{applications.commands}（原生命令所需）
\end{itemize}
...
\begin{itemize}
  ...
  \item ✅ Embed Links
  \item ✅ Attach Files
  \item ✅ Add Reactions（可选但推荐）
  \item ✅ Use External Emojis / Stickers（可选；仅当你需要时）
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Discord 中的执行审批}：Discord 支持私信中执行审批的\textbf{按钮 UI}（允许一次 / 始终允许 / 拒绝）。\texttt{/approve  ...} 仅用于转发的审批，不会解析 Discord 的按钮提示。如果你看到 \texttt{❌ Failed to submit approval: Error: unknown approval id} 或 UI 从未出现，请检查：
...
表情可以是 unicode（例如 \texttt{✅}）或自定义表情语法如 \texttt{<:party\_blob:1234567890>}。
...
❗ \textbf{重要}：请妥善保管 App Secret，不要分享给他人。
...
⚠️ \textbf{重要提醒}：在配置事件订阅前，请务必确保已完成以下步骤：
...
⚠️ \textbf{注意}：如果网关未启动或渠道未添加，长连接设置将保存失败。
...
\begin{itemize}
  ...
  \item \texttt{"ack"} — 机器人发送确认反应（处理时显示 👀）（默认）
...
\textbf{⚠️ 重要：} 添加访问控制（\texttt{allowFrom} 或 \texttt{allowedRoles}）以防止未授权用户触发机器人。\texttt{requireMention} 默认为 \texttt{true}。
...
\begin{itemize}
  \item \texttt{emoji}（字符串）：用于确认的表情（例如"👀"、"✅"、"📨"）。为空或省略 = 功能禁用。
