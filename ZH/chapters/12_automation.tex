...
\begin{itemize}
  \item \textbf{\emoji{💾} session-memory}：当你发出 \texttt{/new} 时将会话上下文保存到智能体工作区（默认 \texttt{\textasciitilde{}/.openclaw/workspace/memory/}）
  \item \textbf{\emoji{📝} command-logger}：将所有命令事件记录到 \texttt{\textasciitilde{}/.openclaw/logs/commands.log}
  \item \textbf{\emoji{🚀} boot-md}：当 Gateway 网关启动时运行 \texttt{BOOT.md}（需要启用内部 hooks）
\end{itemize}
...
\begin{itemize}
  \item \textbf{\texttt{emoji}}：CLI 的显示表情符号（例如 \texttt{"\emoji{💾}"}）
