...
⚠️ \textbf{Bun 警告（WhatsApp + Telegram）：}Bun 与这些渠道存在已知问题。如果你使用 WhatsApp 或 Telegram，请使用 \textbf{Node} 运行 Gateway 网关。
...
\begin{itemize}
  \item GitHub 更名：\texttt{github.com/openclaw/openclaw} ✅
  \item X 账号 \texttt{@openclaw} 获得金色认证标记 💰
  \item npm 包以新名称发布
...
\begin{itemize}
  \item 🦿 腿（Reachy Mini 已下单！）
  \item 👂 耳朵（Brabble 语音守护进程开发中）
  \item 🏠 一个可控制的智能家居（KNX + openhue）
  \item 🌍 统治世界（延伸目标）
\end{itemize}
