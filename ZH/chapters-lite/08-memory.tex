...
\vspace{0.5em}
\noindent\textit{上一章我们了解了 Skill 如何扩展智能体的能力。但智能体如何在多次对话之间“记住”重要信息？
本章将介绍 OpenClaw 的 Memory（记忆）系统——基于 Markdown 文件的透明化、可控的持久化记忆机制，
支持向量语义搜索和全文检索。}
\vspace{0.5em}
...
本章介绍了 Memory 记忆系统的核心机制：

\begin{itemize}
  \item 记忆是智能体工作空间中的\textbf{纯 Markdown 文件}，透明可控
  \item 分层记忆：\textbf{每日日志} + \textbf{长期记忆}（MEMORY.md）
  \item 支持\textbf{向量语义搜索}（sqlite-vec）和\textbf{全文检索}（SQLite FTS5）
  \item 自动记忆刷新机制确保重要信息不会因上下文压缩而丢失
\end{itemize}

\noindent 智能体现在能记住东西了，但它执行的工具命令如何确保安全？下一章我们将介绍 Sandbox（沙箱）隔离机制——安全执行的最后一道防线。
